...
df <- read_csv("data/total_features_to_work_with.csv")
...
# no need to run this, it wasnt the working models, skip to the next part, kept this to show we tried various ways
...
write.csv(filtered_model_data, "data/filtered_model_data.csv")



# the splines part with the working models!! the first one is almost perfect but we used the last one on the script eventually
...
write.csv(df, "data/Dataset_with_death_baseline_full_not_scaled.csv")
model_df <- df %>%
...
write.csv(scaled_df, "data/scaled_df_only_lockdown_after_april.csv")
...
#The final and best model and plot, used death change from prev scaled for better metrics
