...
```
```{r}
full_df <- read.csv("C:/Users/yuval/OneDrive/מסמכים/BGU/4th_semester/data_analysis/research/Final working datasets/Monthly_features_with_violence_normalized.csv")
...
```
```{r}
indicator_choices <- read.csv("C:/Users/yuval/OneDrive/מסמכים/BGU/4th_semester/data_analysis/research/working datasets/indicator_choices.csv")
...
library(rpart.plot)     # עץ החלטה
library(MLmetrics)      # F1 Score
library(caret)          # Confusion Matrix
library(ranger)         # Random Forest

# ===================== שלב 2: קריאת הנתונים וניקוי =====================
data <- read.csv("../Final working datasets/Monthly_features_with_all_data.csv")

# הסרת עמודות עם יותר מ־20% NA
na_thresh <- 0.2 * nrow(data)
na_counts <- sapply(data, function(x) sum(is.na(x)))
data <- data[, na_counts <= na_thresh]

# השלמות חכמות
data$Violence_Value[is.na(data$Violence_Value)] <- median(data$Violence_Value, na.rm = TRUE)
data$Violence_per_million_people[is.na(data$Violence_per_million_people)] <- median(data$Violence_per_million_people, na.rm = TRUE)
data$dv_percent_change_baseline[is.na(data$dv_percent_change_baseline)] <- mean(data$dv_percent_change_baseline, na.rm = TRUE)
...
# ===================== שלב 3: יצירת המטרה =====================
violence_thresh <- quantile(data$Violence_per_million_people, 0.75, na.rm = TRUE)
data$high_violence <- as.factor(ifelse(data$Violence_per_million_people > violence_thresh, 1, 0))

# ===================== שלב 4: חלוקה ל־train/test =====================
set.seed(123)
split_v <- initial_split(data, strata = high_violence)
train_v <- training(split_v)
test_v  <- testing(split_v)

# ===================== שלב 5: Recipe לפיצ'רים נבחרים =====================
rec_viol <- recipe(high_violence ~ Total_lockdown_length + Lockdown_severity +
                     Monthly_income_support + Monthly_workplace_closures_severity +
                     long_lockdown + previous_month_connected_lockdown +
                     lockdown_intensity_per_day,
                   data = train_v) %>%
  step_smote(high_violence)

# ===================== שלב 6: הכנה לאימון =====================
prep_v <- prep(rec_viol)
train_v_bal <- juice(prep_v)

# ===================== שלב 7: אימון Random Forest =====================
rf_model <- rand_forest(mode = "classification", trees = 500) %>%
  set_engine("ranger")

rf_fit <- rf_model %>%
  fit(high_violence ~ ., data = train_v_bal)

# ===================== שלב 8: בדיקה על סט הבדיקה =====================
test_v_proc <- bake(prep_v, new_data = test_v)
preds_v <- predict(rf_fit, new_data = test_v_proc) %>% pull(.pred_class)

# ===================== שלב 9: תוצאות =====================
print(confusionMatrix(preds_v, test_v_proc$high_violence))
cat("F1 Score (Violence): ", F1_Score(y_pred = preds_v, y_true = test_v_proc$high_violence, positive = "1"), "\n")
# ===================== שלב 7: אימון Random Forest =====================
rf_model <- rand_forest(mode = "classification", trees = 500) %>%
...
# ===================== שלב 8: חשיבות משתנים =====================
cat("=== חשיבות משתנים למודל אלימות ===\n")
vi <- rf_fit$fit$variable.importance
print(sort(vi, decreasing = TRUE))
...
# ========== שלב 3: יצירת מטרות ==========
death_thresh <- quantile(data$Monthly_death_rates_per_million_people, 0.75, na.rm = TRUE)
data$high_death <- as.factor(ifelse(data$Monthly_death_rates_per_million_people > death_thresh, 1, 0))

# ========== שלב 4: חלוקה ==========
set.seed(123)
split_d <- initial_split(data, strata = high_death)
train_d <- training(split_d)
test_d  <- testing(split_d)

# ========== שלב 5: בניית recipe ==========
rec_death <- recipe(high_death ~ Total_lockdown_length + Lockdown_severity +
                    Monthly_income_support + Monthly_workplace_closures_severity +
                    long_lockdown + previous_month_connected_lockdown +
                    lockdown_intensity_per_day,
                    data = train_d) %>%
  step_smote(high_death)

# ========== שלב 6: הכנה ואימון ==========
prep_d <- prep(rec_death)
train_d_bal <- juice(prep_d)

model_rf_d <- rand_forest(mode = "classification", trees = 500) %>%
  set_engine("ranger") %>%
  fit(high_death ~ ., data = train_d_bal)

# ========== שלב 7: תחזוי וביצועים ==========
preds_d <- predict(model_rf_d, new_data = test_d) %>% pull(.pred_class)
confusionMatrix(preds_d, test_d$high_death)
F1_Score(preds_d, test_d$high_death, positive = "1")
model_rf_d <- rand_forest(mode = "classification", trees = 500) %>%
  set_engine("ranger", importance = "impurity") %>%
  fit(high_death ~ ., data = train_d_bal)

# ===================== שלב 8: חשיבות משתנים =====================
cat("=== חשיבות משתנים למודל תמותה ===\n")
vi_d <- model_rf_d$fit$variable.importance
print(sort(vi_d, decreasing = TRUE))
